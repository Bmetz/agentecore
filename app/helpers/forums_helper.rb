module ForumsHelper
  ...
  def forum_last_post_info(forum)
    unless (forum.posts.empty?)
      ...
      "#{post.created_at} #{t(:ago_br_by)} "+link_to(post.owner.f, profile_path(post.owner))+"<br/>#{t(:in)} "+link_to(post.topic.title, forum_topic_path(post.topic.forum, post.topic))
    else
      t(:no_posts)
    end
  end
